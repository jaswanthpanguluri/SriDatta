...
export class CowalletComponent implements OnInit {
  ...
  constructor(private titleService: Title, private meta: Meta, private _crud: CurdService, private route: Router) {
    this.titleService.setTitle("SriDutta's - Wallet");
    this.meta.updateTag({ name: 'description', content: "SriDutta's - Wallet" });
    this.meta.updateTag({ name: 'keywords', content: "SriDutta's - Wallet" });
    this.meta.updateTag({ name: 'classification', content: "SriDutta's - Wallet" });
